<template>
  <header
    class="flex justify-between items-center px-6 py-2 w-full bg-[#f7f9fb] dark:bg-slate-900 select-none relative z-[10000] cursor-move"
  ...
  >
    ...
    <div class="flex items-center gap-4">
      <div class="relative group" style="-webkit-app-region: no-drag">
      ...
      <div class="flex items-center gap-2 ml-2">
        <button class="p-2 hover:bg-[#e0e3e5] dark:hover:bg-slate-700 rounded-full transition-colors text-on-surface-variant" style="-webkit-app-region: no-drag">
...
function handleMouseOver(e) {
  ...
  if (targetId === 'btn-minimize' || targetId === 'btn-maximize' || targetId === 'btn-close') {
    isDragArea = false
    e.currentTarget.style.cursor = 'default'
  } else if (target.closest('button')) {
    isDragArea = false
    e.currentTarget.style.cursor = 'default'
  } else if (target.tagName === 'INPUT' || target.tagName === 'A') {
    isDragArea = false
    e.currentTarget.style.cursor = 'default'
  } else {
    isDragArea = true
...
async function handleClose() {
  try {
    ...
    console.error('[AppHeader] close error:', err)
    window.close()
  }
}
